	...
	elseif event.phase == "did" then
		...
		local function normal( event )
			--submitHighScore(IDLEADERBOARDS.tracks2normal,0)
  			composer.gotoScene( "scripts.cenas.2tracks.loading", {effect = "fade",time = 300, params={tabela=IDLEADERBOARDS.tracks2normal,cena="scripts.cenas.2tracks.game",mode="2TRACKS-NORMAL"}} )
  			btn2:removeEventListener( "tap", normal )
  			hideBanner()
		end
		local function hard( event )
			--submitHighScore(IDLEADERBOARDS.tracks2hard,0)
  			composer.gotoScene( "scripts.cenas.2tracks.loading", {effect = "fade",time = 300, params={tabela=IDLEADERBOARDS.tracks2hard,cena="scripts.cenas.2tracks.game",mode="2TRACKS-HARD"}} )
  			btn3:removeEventListener( "tap", hard )
  			hideBanner()
		end
		local function insane( event )
			--submitHighScore(IDLEADERBOARDS.tracks2insane,0)
  			composer.gotoScene( "scripts.cenas.2tracks.loading", {effect = "fade",time = 300, params={tabela=IDLEADERBOARDS.tracks2insane,cena="scripts.cenas.2tracks.game",mode="2TRACKS-INSANE"}} )
