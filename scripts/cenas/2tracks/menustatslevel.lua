...
function scene:create( event )
	...
	local title1 = display.newText( SSTATS.. " " ..S2TRACKS, display.contentCenterX, display.contentHeight / 25 * 2.5, "Bitwise", 50)
...
function scene:show( event )
	...
	elseif event.phase == "did" then
		...
		local function easy( event )
			...
			composer.gotoScene( "scripts.cenas.2tracks.stats", {effect = "slideLeft",time = 300,params={mode="2TRACKS-EASY",origem="menu"}} )
  			btn4:removeEventListener( "tap", easy )
		end
		local function normal( event )
			--submitHighScore(IDLEADERBOARDS.tracks2normal,0)
			composer.gotoScene( "scripts.cenas.2tracks.stats", {effect = "slideLeft",time = 300,params={mode="2TRACKS-NORMAL",origem="menu"}} )
  			btn3:removeEventListener( "tap", normal )
		end
		local function hard( event )
			--submitHighScore(IDLEADERBOARDS.tracks2hard,0)
			composer.gotoScene( "scripts.cenas.2tracks.stats", {effect = "slideLeft",time = 300,params={mode="2TRACKS-HARD",origem="menu"}} )
  			btn2:removeEventListener( "tap", hard )
		end
		local function insane( event )
			--submitHighScore(IDLEADERBOARDS.tracks2insane,0)
			composer.gotoScene( "scripts.cenas.2tracks.stats", {effect = "slideLeft",time = 300,params={mode="2TRACKS-INSANE",origem="menu"}} )
