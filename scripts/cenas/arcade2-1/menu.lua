...
function scene:create( event )
	...
	local title1 = display.newText( "ARCADE 2-1", display.contentCenterX, display.contentHeight / 25 * 2.5, "Bitwise", 50)
...
function scene:show( event )
	...
	elseif event.phase == "did" then
		...
		local function easy( event )
  			composer.gotoScene( "scripts.cenas.arcade2-1.easy", {effect = "fade",time = 300} )
  			btn:removeEventListener( "tap", easy )
		end
		local function normal( event )
  			composer.gotoScene( "scripts.cenas.arcade2-1.normal", {effect = "fade",time = 300} )
  			btn2:removeEventListener( "tap", normal )
		end
		local function hard( event )
  			composer.gotoScene( "scripts.cenas.arcade2-1.hard", {effect = "fade",time = 300} )
  			btn3:removeEventListener( "tap", hard )
		end
		local function insane( event )
  			composer.gotoScene( "scripts.cenas.arcade2-1.insane", {effect = "fade",time = 300} )
