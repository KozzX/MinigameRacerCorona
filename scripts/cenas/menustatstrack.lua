...
function scene:show( event )
	...
	elseif event.phase == "did" then
		...
		local function tracks2( event )
  			composer.gotoScene( "scripts.cenas.2tracks.menustatslevel", {effect = "slideLeft",time = 300} )
  			btn3:removeEventListener( "tap", tracks2 )
  			composer.removeScene( scene )
		end
		local function tracks3( event )
			--native.showAlert( SALERT, SSOON, { "Ok" })
  			composer.gotoScene( "scripts.cenas.3tracks.menustatslevel", {effect = "slideLeft",time = 300} )
  			btn2:removeEventListener( "tap", tracks3 )
  			composer.removeScene( scene )
		end
		local function tracks4( event )
			--native.showAlert( SALERT, SSOON, { "Ok" })
  			composer.gotoScene( "scripts.cenas.4tracks.menustatslevel", {effect = "slideLeft",time = 300} )
